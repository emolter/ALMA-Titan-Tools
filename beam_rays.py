...
Titan_dist = 10.3317282939967 #au. Get from delta column in JPL Horizons
d_shift = 25.7107010 #km/s. Get from deldot column in JPL Horizons
subobslat = 14.05 #degrees. Get from Ob-lat column in JPL Horizons
ccw = 358.2180 #degrees. Get from  NP.ang column in JPL Horizons - counterclockwise from north

##### Model Options #####
showplot = False #Display image of beam overlaid on Titan with latitudes as contours?
diskavg = True #compute disk averaged spectrum?  Note disk average takes ~1 minute to run because of runtime of imval
...
##### Model Parameters #####
cx = [0.0] #cx,cy tell the model where to extract data. Together they form a grid of x,y distances (in km from the center of Titan) at which to extract spectra and compute weights. Note that 0.05 arcsec ~= 350 km at Titan's distance. A disk averaged model forces cx = cy = [0.0]
cy = [-3000.0] #cy = range(-3750,4125,375)
...
def spEx(img,xpix,ypix):
    ...
    #Calculate how far away from cx,cy the pixel center is actually located
...
def computeWeights_diskavg():
    ...
    #Change values of z to the angle corresponding to those radii
    angles = [np.degrees(np.arcsin(float(r)/float(top_atm))) for r in 0.5*(radii[1:] + radii[:-1]))] #top of atmosphere emission angle
...
for i in cy:
    for j in cx:
        ...
        if not diskavg:
            (freq,flux,xcorr_km,ycorr_km) = spEx(img,pixx,pixy) #extract spectrum
            wts,meanlat = computeWeights(j,i,xcorr_km,ycorr_km) #compute weights
        else:
            (freq,flux,xcorr_km,ycorr_km) = spEx_diskavg(img,pixx,pixy) #extract spectrum
